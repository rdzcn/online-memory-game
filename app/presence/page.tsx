...
export default function Presence() {
  ...
  const handlePresenceMessage = useCallback(
    (message: Ably.Types.PresenceMessage) => {
      ...
      setLogs((prev) => [
        ...prev,
        new LogEntry(`action: ${message.action} clientId: ${message.clientId}`),
      ]);
    },
    [],
  );
  ...
  return (
    <>
      <p>
        Presence with Ably allows you to keep track of devices that are present
        on a channel. This is great for tracking if a device is online or
        offline or indicating if a user is in a chat room when using Ably for
        Chat.
      </p>
      <p>
        ...
        in another tab to see more users enter and leave the presence channel.
      </p>
